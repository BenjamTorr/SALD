...
class CrossAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.kv_proj = nn.Linear(self.hidden_size, 2 * self.att_dim, bias=True)
        self.output_proj = nn.Sequential(
            nn.Linear(self.att_dim, self.hidden_size))
    ...
    def forward(self, x, context = None):
        ...
        # q(as well as k and v) -> Batch Size x Number of Patches x Attention Dimension
        q = self.q_proj(x)
        k, v = self.kv_proj(context).split(self.att_dim, dim=-1)
        # Batch Size x Number of Patches x Attention Dimension
        # -> Batch Size x Number of Patches x (Heads * Head Dimension)
        # -> Batch Size x Number of Patches x (Heads * Head Dimension)
        # -> Batch Size x Heads x Number of Patches x Head Dimension
...
class TransformerLayerWCrossAtt(nn.Module):
    r"""
    ...
    def forward(self, x, condition, context = None):
        ...
        scale_shift_params = self.adaptive_norm_layer(condition).chunk(9, dim=1)
        # Chunk into 6 different parameters
        (pre_attn_shift, pre_attn_scale, post_attn_scale,
         pre_cattn_shift, pre_cattn_scale, post_cattn_scale,
        ...
        cross_attn_norm_output = (self.cross_attn_norm(out) * (1 + pre_cattn_scale.unsqueeze(1))
                            + pre_cattn_shift.unsqueeze(1))
        
        out = out + post_cattn_scale.unsqueeze(1) * self.cross_attn_block(cross_attn_norm_output, context)
...
def get_patch_position_embedding(pos_emb_dim, grid_size, device):
    assert pos_emb_dim % 2 == 0, 'Position embedding dimension must be divisible by 2'
    grid_l_positions = torch.arange(grid_size, dtype=torch.float32, device=device)
    # grid_l_positions -> (Number of patch tokens,)
    # factor = 10000^(2i/d_model)
    factor = 10000 ** ((torch.arange(
        start=0,
        end=pos_emb_dim // 2,
        dtype=torch.float32,
        device=device) / (pos_emb_dim // 2))
    )

    grid_l_emb = grid_l_positions[:, None].repeat(1, pos_emb_dim // 2) / factor
    grid_l_emb = torch.cat([torch.sin(grid_l_emb), torch.cos(grid_l_emb)], dim=-1)
...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        patch_dim = self.seq_channels * self.patch_size
    ...
    def forward(self, x):
        grid_size = self.seq_len // self.patch_size
        ### HERE


        
        # B, C, l -> B, (Patches along height * Patches along width), Patch Dimension
        # Number of tokens = Patches along height * Patches along width
        out = rearrange(x, 'b c (sl pl) -> b sl (pl c)',
                        pl=self.patch_size)
...
class dit_cat(nn.Module):
    def __init__(self, seq_len, seq_channels, config):
        super().__init__()
        ...
        self.seq_channels_out = config['channels_out']
        self.class_emb_dim = config['class_emb_dim']

        self.timestep_emb_dim = config['timestep_emb_dim']

        # Number of patches along height and width
        self.sl = self.seq_len // self.patch_size
        

        # Patch Embedding Block
        self.patch_embed_layer = PatchEmbedding(seq_len = self.seq_len,
                                                seq_channels = self.seq_channels,
                                                patch_size=self.patch_size,
                                                hidden_size=self.hidden_size)
        
        # Patch Embedding Block
        self.patch_embed_layer_context = PatchEmbedding(seq_len = self.seq_len,
                                                seq_channels = self.seq_channels // 2,
                                                patch_size=self.patch_size,
                                                hidden_size=self.hidden_size)
        ...
        self.c_proj =  nn.Sequential(
            nn.Linear(self.class_emb_dim, self.hidden_size),
            nn.SiLU(),
            nn.Linear(self.hidden_size, self.hidden_size)
        )
        ...
        self.layers = nn.ModuleList([
            TransformerLayerWCrossAtt(config) for _ in range(num_layers)
        ])
        ...
        self.proj_out = nn.Linear(self.hidden_size,
                                  self.patch_size  * self.seq_channels_out)
    ...
    def forward(self, x, t, class_encoding, context = None):
        # Patchify
        ...
        context_patched = self.patch_embed_layer_context(context) if context is not None else None

        # Go through the transformer layers
        for layer in self.layers:
            out = layer(out, t_emb, context_patched)
        ...
        # Unpatchify
        # (B,patches,hidden_size) -> (B,patches,seq_channels * patch_size)
        out = self.proj_out(out)
        #'b c (sl pl) -> b sl (pl c)'
        out = rearrange(out, 'b sl (pl c) -> b c (sl pl)',
                        sl=self.sl,
                        pl=self.patch_size,
                        c =self.seq_channels_out)
        return out