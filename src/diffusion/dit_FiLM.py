...
class dit_film(nn.Module):
    def __init__(self, seq_len, seq_channels, config):
        ...
        # FiLM-style context projection (context shape: B x L_s)
    ...
    def forward(self, x, t, class_encoding, context = None):
        ...
        context_emb = self.context_proj(context) if context is not None else 0
        t_emb = self.t_proj(t_emb) + self.c_proj(class_encoding) + context_emb
